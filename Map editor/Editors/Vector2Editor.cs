...
{
    class Vector2Editor : ValueEditor
    {
        NumericUpDown[] nmrValHolders;

        public Vector2Editor()
        {
            nmrValHolders = new NumericUpDown[2];
            for (int i = 0; i < 2; i++)
            {
        ...
        {
            SetValue(new Vector2(Convert.ToSingle(nmrValHolders[0].Value), Convert.ToSingle(nmrValHolders[1].Value)));
        }

        public override void SetValue(object value)
        {
            nmrValHolders[0].Value = Convert.ToDecimal(((Vector2)value).X);
            nmrValHolders[1].Value = Convert.ToDecimal(((Vector2)value).Y);
            base.SetValue(value);
        }
